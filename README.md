...
After taking a pull, create a local `.env` file from `.env.example` and replace every placeholder with your actual local values:

```bash
cp .env.example .env
```

The `.env` file is intentionally not committed. Docker Compose reads it during startup, and the NiFi container receives those values through `env_file` so NiFi Expression Language can resolve them.
...
```bash
docker compose --env-file .env -f docker-compose.nifi.yml up -d
```

Start both together:

```bash
docker compose --env-file .env -f docker-compose.postgres.yml -f docker-compose.nifi.yml up -d
```

Both compose files use the shared Docker network `docker-shared-network`. When NiFi runs in Docker and connects to the PostgreSQL compose service, set `POSTGRES_HOST=postgres` in `.env`.

## Flow Summary

1. `ListFile` scans `${CONTAINER_BASE_PATH}` for `*.zip`.
2. A parent UUID is generated and inserted into `file_metadata` with status `SUBMITTED`.
3. `FetchFile` fetches the ZIP and moves the source file to `${CONTAINER_BASE_PATH}${ARCHIVE_ZIP_PATH}`.
...
8. Each child file receives a UUID, is inserted with status `SUBMITTED`, and is written to `${CONTAINER_BASE_PATH}${FILE_ARCHIVE_PATH}`.
